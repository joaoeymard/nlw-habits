...
export function HabitsList({ date, onCompletedChanged }: HabitsListProps) {
  ...
  return (
    <div className="mt-6 flex flex-col gap-3">
      {habitsInfo?.possibleHabits.map((habit, ind) => {
        return (
          <Checkbox.Root
            key={ind}
          ...
          >
            ...
            <span className="font-semibold text-xl text-while leading-tight group-data-[state=checked]:line-through group-data-[state=checked]:text-zinc-400">
              {habit.title}
            </span>
          </Checkbox.Root>
        );
